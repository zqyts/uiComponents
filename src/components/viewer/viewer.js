...
(function(factory) {
  ...
  // Supports
  var SUPPORT_TRANSITION = typeof ELEMENT_VIEWER.style.transition !==
    'undefined';
  ...
  function Viewer(element, options) {
    ...
    this.options = $.extend({}, Viewer.DEFAULTS, $.isPlainObject(options) &&
      options);
    this.isImg = false;
    this.isBuilt = false;
    this.isShown = false;
  ...
  Viewer.prototype = {
    ...
    build: function() {
      ...
      $title.addClass(!options.title ? CLASS_HIDE : getResponsiveClass(
        options.title));

      $toolbar.addClass(!options.toolbar ? CLASS_HIDE :
        getResponsiveClass(options.toolbar));
      $toolbar.find('li[class*=zoom]').toggleClass(CLASS_INVISIBLE, !
        options.zoomable);
      $toolbar.find('li[class*=flip]').toggleClass(CLASS_INVISIBLE, !
        options.scalable);

      if (!options.rotatable) {
        $toolbar.find('li[class*=rotate]').addClass(CLASS_INVISIBLE).appendTo(
          $toolbar);
      }

      $navbar.addClass(!options.navbar ? CLASS_HIDE : getResponsiveClass(
        options.navbar));
    ...
    resetList: function() {
      this.$list.empty().removeClass(CLASS_TRANSITION).css('margin-left',
        0);
    },
    ...
    mousemove: function(event) {
      ...
      if (action) {
        ...
        if (action === 'move' && options.transition && $image.hasClass(
            CLASS_TRANSITION)) {
    ...
    show: function() {
      ...
      this.$element.one(EVENT_SHOWN, $.proxy(function() {
        this.view(this.target ? this.$images.index(this.target) :
          this.index);
        this.target = false;
      }, this));
      ...
      if (options.transition) {
        ...
        $viewer.one(EVENT_TRANSITIONEND, $.proxy(this.shown, this)).addClass(
          CLASS_IN);
      } else {
    ...
    hide: function() {
      ...
      if (this.isViewed && options.transition) {
        ...
        this.$image.one(EVENT_TRANSITIONEND, $.proxy(function() {
          $viewer.one(EVENT_TRANSITIONEND, $.proxy(this.hidden,
    ...
    zoomTo: function(ratio, hasTooltip, _event, _zoomable) {
      ...
      if (isNumber(ratio) && this.isViewed && !this.isPlayed && (
        ...
        if (_event && (originalEvent = _event.originalEvent)) {
          ...
          center = originalEvent.touches ? getTouchesCenter(originalEvent
            .touches) : {
    ...
    rotateTo: function(degree) {
      ...
      if (isNumber(degree) && this.isViewed && !this.isPlayed && this.options
        .rotatable) {
        image.rotate = degree;
        this.renderImage();
      }
    ...
    play: function() {
      ...
      this.$items.each(function(i) {
        ...
        var $image = $('<img src="' + $img.data('originalUrl') +
          '" alt="' + $img.attr('alt') + '">');

        total++;

        $image.addClass(CLASS_FADE).toggleClass(CLASS_TRANSITION,
          options.transition);
    ...
    full: function() {
      ...
      this.$viewer.addClass(CLASS_FIXED).removeAttr('style').css(
